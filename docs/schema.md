...
| `search(query: String!, pagination)` | Query string + Pagination | `SearchResult!` |
...
Relevance-ranked full-text search across published Contents and Tags
(`SearchHelper` in Entity mode — unpublished entries are excluded).

| Field | Type | Description |
|---|---|---|
| `contents` | `[Content!]!` | Matching contents, ordered by relevance |
| `tags` | `[Tag!]!` | Matching tags, ordered by relevance |
...
| `TypeFactory::searchResult()` | `SearchResultType` |
