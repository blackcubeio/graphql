...
class TypeFactory
{
    ...
    {
        return self::$types['SearchResult'] ??= new SearchResultType();
    }
    ...
    public static function elastic(): ?UnionType
    {
        ...
        if (empty(self::$elasticTypes) === true) {
            return null;
        }
    ...
    private static function buildObjectTypeFromSchema(ElasticSchema $schema): ?ObjectType
    {
        ...
        if (($decoded instanceof \stdClass) === false) {
            return null;
        }

        if (self::$fastSchema === true) {
        ...
        $fields = [
            'elasticSchemaId' => [
                ...
                'resolve' => fn (array $values) => $values['_elasticSchemaId'],
            ],
        ];
        ...
        foreach ($properties as $key => $property) {
            ...
            if ($type === 'object') {
                $fields[$key] = ['type' => Type::string(), 'resolve' => fn (array $values) => isset($values[$key]) === true ? json_encode($values[$key]) : null];
                continue;
            }
            ...
            if ($type === 'string' && $format === 'file') {
                $fields[$key] = [
                    'type' => Type::string(),
                    'resolve' => static function (array $values, array $args, array $context) use ($key): ?string {
            ...
            if ($type === 'string' && $format === 'files') {
                $fields[$key] = [
                    ...
                    'resolve' => static function (array $values, array $args, array $context) use ($key): array {
                        $raw = $values[$key] ?? '';
                        if ($raw === '') {
                            return [];
                        }
                        ...
                        foreach (preg_split('/\s*,\s*/', $raw, -1, PREG_SPLIT_NO_EMPTY) as $path) {
                            ...
                            $urls[] = str_starts_with($url, '/') === true ? 'https://'.$host.$url : $url;
                        }
                        return $urls;
                    },
                ];
