    ...
    public function __construct()
    {
        parent::__construct([
            ...
            'fields' => fn () => [
                ...
                'dateCreate' => [
                    'type' => Type::string(),
                    'resolve' => fn (Menu $menu) => $menu->getDateCreate()?->format('Y-m-d H:i:s'),
                ],
                'dateUpdate' => [
                    'type' => Type::string(),
                    'resolve' => fn (Menu $menu) => $menu->getDateUpdate()?->format('Y-m-d H:i:s'),
                ],
                'children' => [
                    'type' => Type::nonNull(Type::listOf(Type::nonNull(TypeFactory::menu()))),
                    'resolve' => fn (Menu $menu) => Menu::query()->children()->andWhere(['>', 'left', $menu->left])->andWhere(['<', 'right', $menu->right])->andWhere(['level' => $menu->level + 1])->all(),
                ],
                'parent' => [
                    'type' => TypeFactory::menu(),
                    'resolve' => fn (Menu $menu) => Menu::query()->parent()->andWhere(['<', 'left', $menu->left])->andWhere(['>', 'right', $menu->right])->andWhere(['level' => $menu->level - 1])->one(),
                ],
                'language' => [
                    'type' => TypeFactory::language(),
                    'resolve' => fn (Menu $menu) => $menu->getLanguageQuery()->one(),
                ],
            ],
        ]);
    }
